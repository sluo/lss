...
def main(args):
  ...
  plot(ob,Sampling(nl,0.1*dl,0.1*fl*dl),sa,cmin=0.0,cmax=1.0,title='obj')
  ...
  if showLocalMinimum:
    ...
    plot(obj,sl,sa,cmin=0.0,cmax=1.0,title='aaa')
    lmin = 1.0e6
    aopt = 0.0
    for ia in range(na):
      obji = obj[ia][nl/2]
  ...
  #points(st,ds,do,title='ds_do')
  cmax = 1.1*max(max(ds),max(do)); cmin = -cmax
...
def points(s,x1,x2=None,cmin=0.0,cmax=0.0,title=None):
  panel = PlotPanel()
  ...
  if x2 is not None:
    ...
    #point2.setLineStyle(PointsView.Line.DOT)
