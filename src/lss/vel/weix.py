...
st = Sampling(3001,0.0012,ft)
nz,nx,nt = sz.count,sx.count,st.count
dz,dx,dt = sz.delta,sx.delta,st.delta
fpeak = 10.0 # Ricker wavelet peak frequency

kzs,kxs = 0,nx/2 # source
#kzr,kxr = fillint(0,nx),rampint(0,1,nx) # receivers
kzr,kxr = fillint(nz-1,nx),rampint(0,1,nx) # receivers
...
# TODO finish testX for reflection geometry
# TODO modify BornWavefield to save both wavefields
# TODO Gaussian anomaly with reflector
# TODO Gaussian anomaly with velocity gradient
...
def test():
  ...
  d = aw.getWavefield(fillint(kzs,nx),rampint(0,1,nx))
  plot(d,title='backpropagated data',png='b')


def testX():
  
  # XXX works only for transmission geometry

  alpha = 0.0 # plane wave angle
  v,c = getModel()
  aw = AcousticWavefieldX(sz,sx,st)
  pzs,pxs = fillint(kzs,nx),rampint(0,1,nx)
  plot(v,cmap=jet,title='true velocity',png='v')
  plot(c,cmap=jet,title='init velocity',png='c')

  # Recorded data
  source = AcousticWavefieldX.PlaneWaveSource(alpha,fpeak,dx,kzs,kxs,v)
  aw.forwardPropagate(source,v)
  d = aw.getWavefield(kzr,kxr)
  GaussianTaper.apply2(d,d)
  plot(d,perc=100,title='dataX',png='d')

  # Back-propagate recorded data
  source = AcousticWavefieldX.AdjointSource(kzr,kxr,st,d)
  aw.backPropagate(source,c)
  d = aw.getWavefield(pzs,pxs)
  u = aw.getWavefield()
  plot(d,title='backpropagated dataX',png='b')

  # Penalize back-propagated data (pseudosource)
  w = makePenalty(alpha,c)
  mul(w,d,d)
  plot(w,title='penalty',png='w')
  plot(d,title='penalized backpropagated',png='p')
  GaussianTaper.apply2(d,d)
  plot(d,title='tapered penalized backpropagated',png='tp')

  # Forward-propagate pseudosource
  source = AcousticWavefieldX.DefaultSource(pzs,pxs,st,d)
...
def makePenalty(alpha,v):
  ...
  for ix in range(nx):
    ...
    for it in range(nt):
      t = ft+it*dt
      w[ix][it] = f(t-px)
  l = sum(v)/nx/nz/fpeak/dx # average wavelength in samples
  #print l
  RecursiveGaussianFilter(2.0*l).apply0X(w,w)
  return w
...
def laplacian(x):
  ...
  for i2 in range(1,n2-1):
    for i1 in range(1,n1-1):
      y[i2][i1] = x[i2][i1+1]+x[i2+1][i1]+\
                  x[i2][i1-1]+x[i2-1][i1]-4.0*x[i2][i1]
  return y
...
def gaussianModel():
  ...
  t = zerofloat(nz,nx)
  t[nx/2][nz/2] = 1.0
  RecursiveGaussianFilter(16.0).apply00(t,t)
  t = normalize(t)
  t = mul(t,500.0)
  v = add(t,v)
  #plot(v)
